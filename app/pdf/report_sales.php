<?php
	...
	$pdf->SetTextColor(39,39,51);
	$pdf->Cell(0,10,iconv("UTF-8", "ISO-8859-1",strtoupper('REPORTE GENERAL DE VENTAS')),0,1,'C');
    $pdf->SetFont('Arial','',10);
    $pdf->Cell(0,7,iconv("UTF-8", "ISO-8859-1",'Desde: '.date("d/m/Y", strtotime($fecha_inicio)).'  Hasta: '.date("d/m/Y", strtotime($fecha_fin))),0,1,'C');
    $pdf->Ln(5);

    // Tabla Cabecera (Más ancha para meter los Bs)
	...
	$pdf->Cell(50,8,iconv("UTF-8", "ISO-8859-1",'Vendedor'),1,0,'C',true);
	...
	/*---------- Consultar Ventas en Rango de Fechas (Con Tasa BCV) ----------*/
    $tabla_consulta = "venta INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta_fecha BETWEEN '$fecha_inicio' AND '$fecha_fin' ORDER BY venta_fecha ASC, venta_id ASC";
    $campos_consulta = "venta.venta_id, venta.venta_codigo, venta.venta_fecha, venta.venta_total, venta.venta_tasa_bcv, usuario.usuario_nombre, usuario.usuario_apellido";
    ...
    if(count($ventas) > 0){
        foreach($ventas as $row){
            ...
            $pdf->Cell(25,7,iconv("UTF-8", "ISO-8859-1",date("d/m/Y", strtotime($row['venta_fecha']))),'L',0,'C');
            $pdf->Cell(35,7,iconv("UTF-8", "ISO-8859-1",$row['venta_codigo']),'L',0,'C');
            $pdf->Cell(50,7,iconv("UTF-8", "ISO-8859-1", substr($row['usuario_nombre']." ".$row['usuario_apellido'], 0, 25)),'L',0,'L');
            $pdf->Cell(20,7,iconv("UTF-8", "ISO-8859-1",$str_tasa),'L',0,'C');
            $pdf->Cell(20,7,iconv("UTF-8", "ISO-8859-1",number_format($row['venta_total'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'L',0,'R');
        ...
        $pdf->Cell(185,10,iconv("UTF-8", "ISO-8859-1","No se encontraron ventas en este rango de fechas."),'LBR',0,'C');
        $pdf->Ln(10);
    }

    // Pie de Tabla con Totales
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(140,8,iconv("UTF-8", "ISO-8859-1",'TOTAL VENDIDO EN EL PERIODO:'),'T',0,'R');
	...
	$pdf->Output("I","Reporte_Ventas_".$fecha_inicio."_al_".$fecha_fin.".pdf",true);
?>