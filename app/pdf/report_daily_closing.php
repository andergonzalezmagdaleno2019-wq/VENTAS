<?php
	...
	$pdf->SetTextColor(39,39,51);
	$pdf->Cell(0,10,iconv("UTF-8", "ISO-8859-1",strtoupper('REPORTE DE CIERRE DIARIO')),0,1,'C');
    $pdf->SetFont('Arial','',10);
	...
	$pdf->Cell(20,8,iconv("UTF-8", "ISO-8859-1",'Hora'),1,0,'C',true);
	$pdf->Cell(35,8,iconv("UTF-8", "ISO-8859-1",'Código'),1,0,'C',true);
	$pdf->Cell(45,8,iconv("UTF-8", "ISO-8859-1",'Vendedor'),1,0,'C',true);
    ...
    $tabla_consulta = "venta INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta_fecha = '$fecha_hoy' ORDER BY venta_hora ASC, venta_id ASC";
    ...
    if(count($ventas) > 0){
        foreach($ventas as $row){
            ...
            $pdf->Cell(20,7,iconv("UTF-8", "ISO-8859-1", date("h:i A", strtotime($row['venta_hora']))),'L',0,'C');
            $pdf->Cell(35,7,iconv("UTF-8", "ISO-8859-1",$row['venta_codigo']),'L',0,'C');
            $pdf->Cell(45,7,iconv("UTF-8", "ISO-8859-1", substr($row['usuario_nombre']." ".$row['usuario_apellido'], 0, 25)),'L',0,'L');
	...
	$pdf->Cell(120,8,iconv("UTF-8", "ISO-8859-1",'TOTAL VENTAS DEL DÍA:'),'T',0,'R');
	$pdf->Cell(30,8,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($total_general_usd,MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR)),'T',0,'R');
    $pdf->SetTextColor(32,100,210); // Resaltar Bolívares
    $pdf->Cell(35,8,iconv("UTF-8", "ISO-8859-1",'Bs. '.number_format($total_general_bs,2,',','.')),'T',0,'R');
	$pdf->Ln(12);
