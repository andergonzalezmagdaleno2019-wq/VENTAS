<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
            ...
            // NUEVO: Capturamos el rol (Por defecto 2: Vendedor, si algo falla)
			...
			$usuario_datos_reg=[
                ...
                ["campo_nombre"=>"rol_id","campo_marcador"=>":Rol","campo_valor"=>$rol] // Registramos el rol
			];
		...
		/*----------  Controlador listar usuario (AHORA MUESTRA EL ROL)  ----------*/
		public function listarUsuarioControlador($pagina,$registros,$url,$busqueda){
			...
			if($total>=1 && $pagina<=$numeroPaginas){
				...
				foreach($datos as $rows){
                    ...
                    // TRADUCIR EL ROL_ID A UN TEXTO VISUAL CON COLORES
                    $rol_etiqueta = '<span class="tag is-light">Desconocido</span>';
                    if(isset($rows['rol_id'])){
                        if($rows['rol_id'] == 1){ 
                            $rol_etiqueta = '<span class="tag is-danger is-light font-weight-bold">Administrador</span>'; 
                        } elseif($rows['rol_id'] == 2){ 
                            $rol_etiqueta = '<span class="tag is-info is-light font-weight-bold">Vendedor / Cajero</span>'; 
                        } elseif($rows['rol_id'] == 3){ 
                            $rol_etiqueta = '<span class="tag is-success is-light font-weight-bold">Supervisor</span>'; 
                        }
                    }
				...
				$tabla.='<tr class="has-text-centered" ><td colspan="8">No hay registros en el sistema</td></tr>';
			}
		...
		/*----------  Controlador actualizar usuario (AHORA GUARDA EL ROL)  ----------*/
		public function actualizarUsuarioControlador(){
            ...
            // NUEVO: Capturar Rol (si no lo enviaron porque no es admin, dejamos el que ya tenía)
			...
			$usuario_datos_up=[
				...
				["campo_nombre"=>"usuario_clave","campo_marcador"=>":Clave","campo_valor"=>$clave],
				["campo_nombre"=>"caja_id","campo_marcador"=>":Caja","campo_valor"=>$caja],
                ["campo_nombre"=>"rol_id","campo_marcador"=>":Rol","campo_valor"=>$rol] // ACTUALIZAMOS ROL AQUÍ
			];
        ...
        /*----------  Controladores de fotos (sin alterar)  ----------*/
