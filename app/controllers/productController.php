	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
			...
			if($this->verificarDatos("[a-zA-Z0-9- ]{1,70}",$codigo)){
				$alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Formato de código inválido","icono"=>"error"]; return json_encode($alerta); exit();
			}

			$check_codigo=$this->ejecutarConsulta("SELECT producto_codigo FROM producto WHERE producto_codigo='$codigo'");
			if($check_codigo->rowCount()>0){
				$alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"El código ya existe","icono"=>"error"]; return json_encode($alerta); exit();
			}
        ...
        public function actualizarEstadoProductoControlador(){
            ...
            if($datos->rowCount()<=0){ 
                $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Producto no encontrado","icono"=>"error"]; return json_encode($alerta); exit();
            }
            $datos = $datos->fetch();
		...
		public function eliminarProductoControlador(){
		    ...
		    if($datos->rowCount()<=0){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Producto no encontrado","icono"=>"error"]; return json_encode($alerta); exit(); }
		    $datos=$datos->fetch();
		    ...
		    if($eliminarProducto->rowCount()==1){
		    	...
		    	if(is_file("../views/productos/".$datos['producto_foto'])){ chmod("../views/productos/".$datos['producto_foto'],0777); unlink("../views/productos/".$datos['producto_foto']); }
		...
		public function actualizarProductoControlador(){
            ...
            if($datos['producto_codigo']!=$codigo){
			    $check_codigo=$this->ejecutarConsulta("SELECT producto_codigo FROM producto WHERE producto_codigo='$codigo'");
			    if($check_codigo->rowCount()>0){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"El código ya existe","icono"=>"error"]; return json_encode($alerta); exit(); }
            }
		...
		public function actualizarFotoProductoControlador(){
		    ...
		    $datos=$datos->fetch();
    		$img_dir="../views/productos/";
    		if($_FILES['producto_foto']['name']=="" && $_FILES['producto_foto']['size']<=0){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Seleccione una foto","icono"=>"error"]; return json_encode($alerta); exit(); }
	        ...
	        if(mime_content_type($_FILES['producto_foto']['tmp_name'])!="image/jpeg" && mime_content_type($_FILES['producto_foto']['tmp_name'])!="image/png"){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Formato incorrecto","icono"=>"error"]; return json_encode($alerta); exit(); }
	        
            if($datos['producto_foto']!=""){ $foto=explode(".", $datos['producto_foto']); $foto=$foto[0]; }else{ $foto=str_ireplace(" ","_",$datos['producto_nombre'])."_".rand(0,100); }
	        switch(mime_content_type($_FILES['producto_foto']['tmp_name'])){ case 'image/jpeg': $foto=$foto.".jpg"; break; case 'image/png': $foto=$foto.".png"; break; }
	        chmod($img_dir,0777);
	        if(!move_uploaded_file($_FILES['producto_foto']['tmp_name'],$img_dir.$foto)){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Error al subir imagen","icono"=>"error"]; return json_encode($alerta); exit(); }
	        if(is_file($img_dir.$datos['producto_foto']) && $datos['producto_foto']!=$foto){ chmod($img_dir.$datos['producto_foto'], 0777); unlink($img_dir.$datos['producto_foto']); }
        ...
        public function eliminarFotoProductoControlador(){
    		...
    		$img_dir="../views/productos/";
    		chmod($img_dir,0777);
    		if(is_file($img_dir.$datos['producto_foto'])){ chmod($img_dir.$datos['producto_foto'],0777); if(!unlink($img_dir.$datos['producto_foto'])){ $alerta=["tipo"=>"simple","titulo"=>"Error","texto"=>"Error al borrar archivo","icono"=>"error"]; return json_encode($alerta); exit(); } }
