...
<div class="container pb-6 pt-6">
    <?php
        ...
        /* --- CÁLCULO DE INDICADORES FINANCIEROS SIMPLIFICADOS --- */
        ...
        <div class="column is-4">
            <div class="box has-background-info-light" style="border-left: 5px solid #209cee; height: 100%;">
                <div class="level is-mobile">
                    ...
                    <div class="level-item has-text-centered"><i class="fas fa-money-bill-wave fa-3x has-text-info"></i></div>
                </div>
            </div>
        </div>
        ...
        <div class="column is-4">
            <div class="box has-background-danger-light" style="border-left: 5px solid #ff3860; height: 100%;">
                <div class="level is-mobile">
                    ...
                    <div class="level-item has-text-centered"><i class="fas fa-exclamation-circle fa-3x has-text-danger"></i></div>
                </div>
            </div>
        </div>
        ...
        <div class="column is-4">
            <div class="box has-background-warning-light" style="border-left: 5px solid #ffdd57; height: 100%;">
                <div class="level is-mobile">
                    ...
                    <div class="level-item has-text-centered"><i class="fas fa-calendar-alt fa-3x has-text-warning-dark"></i></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="box">
        <h3 class="title is-5 has-text-link mb-4"><i class="fas fa-file-invoice-dollar"></i> Lista de Deudas Pendientes</h3>
        <?php
                ...
                <tbody>
                    <?php
                        foreach($datos as $c){
                            ...
                            if($dias < 0){ $clase_color = "has-text-danger has-text-weight-bold"; $mensaje = "Vencida hace " . abs($dias) . " días"; $icono = "fa-exclamation-triangle"; } 
                            elseif($dias <= 3){ $clase_color = "has-text-warning has-text-weight-bold"; $mensaje = "Vence en $dias días"; $icono = "fa-clock"; } 
                            else { $clase_color = "has-text-info"; $mensaje = "Al día ($dias días)"; $icono = "fa-check-circle"; }
                            $tag_color = ($c['compra_estado_pago'] == "Pendiente") ? "is-danger" : "is-info";
                    ?>
                        <tr class="has-text-centered">
                            <td style="vertical-align: middle;"><?php echo $c['compra_codigo']; ?></td>
                            <td style="vertical-align: middle;"><?php echo $c['proveedor_nombre']; ?></td>
                            <td style="vertical-align: middle;"><span class="tag <?php echo $tag_color; ?> is-light is-rounded"><?php echo $c['compra_estado_pago']; ?></span></td>
                            <td class="<?php echo $clase_color; ?>" style="vertical-align: middle;"><i class="fas <?php echo $icono; ?>"></i><br><?php echo date("d/m/Y", strtotime($c['compra_fecha_vencimiento'])); ?><br><small><?php echo $mensaje; ?></small></td>
                            <td class="is-size-5 has-text-weight-bold has-text-danger-dark" style="vertical-align: middle;"><?php echo number_format($c['compra_saldo_pendiente'], 2); ?> $</td>
                            <td style="vertical-align: middle;">
                                <div class="buttons is-centered">
                                    <button class="button is-success is-small is-rounded" onclick="abrirModalAbono('<?php echo $c['compra_id']; ?>', '<?php echo $c['compra_codigo']; ?>', '<?php echo $c['compra_saldo_pendiente']; ?>')"><i class="fas fa-hand-holding-usd"></i>&nbsp; Abonar</button>
                                    <button class="button is-info is-small is-rounded" onclick="verHistorialAbonos('<?php echo $c['compra_id']; ?>', '<?php echo $c['compra_codigo']; ?>')"><i class="fas fa-history"></i></button>
                                </div>
                            </td>
                        </tr>
                    <?php } ?>
        ...
        <?php } else { ?>
            <div class="notification is-success is-light has-text-centered"><i class="fas fa-check-circle fa-2x mb-2"></i><br><strong>¡Todo al día!</strong> No tienes deudas pendientes con proveedores.</div>
        <?php } ?>
    </div>
</div>
    ...
    <div class="modal-card">
        ...
        <section class="modal-card-body">
            <form class="FormularioAjax" action="<?php echo APP_URL; ?>app/ajax/compraAjax.php" method="POST" autocomplete="off">
                ...
                <p class="has-text-centered mt-4"><button type="submit" class="button is-info is-rounded"><i class="fas fa-check"></i> Confirmar Pago</button></p>
            </form>
        </section>
    </div>
</div>
